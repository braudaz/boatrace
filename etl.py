...
def update_with_xml(game_data, course_data, xml_type, src_type, src_path):
	if src_type == 'url':
		file = './tmp/scrape.txt'
		if not scrape_url(src_path, file): return
	else:
		file = src_path
...
def get_data_record(game_id, idx, id_data, game_data, course_data, for_train):
...
def get_statistics(game_id, idx, id_data, game_data, course_data):
	game_info = game_data[game_id]
	before_info = game_info['before']
